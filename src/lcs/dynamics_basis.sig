...
sig
  ...
  structure M : LCS_MACHINE
    where type 'a Cl.Abt.Operator.Arity.Valence.Spine.t = 'a list

  structure Sig : LCS_SIGNATURE
    where type valence = O.L.V.Arity.Valence.t

  sharing type Sig.symbol = M.Cl.Abt.Symbol.t
  sharing type Sig.metavariable = M.Cl.Abt.Metavariable.t
  sharing type Sig.sort = O.L.V.Arity.Valence.Sort.t
  sharing type Sig.term = M.Cl.Abt.abt
  sharing type M.Cl.Abt.Operator.Arity.Valence.Sort.t = O.Sort.t
